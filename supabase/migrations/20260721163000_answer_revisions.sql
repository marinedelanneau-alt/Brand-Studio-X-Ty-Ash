...
begin
  insert into public.user_answers (
    user_id, project_id, module_key, submodule_key, exercise_key,
    question_key, field_key, answer_value, source_version_id,
    client_updated_at, client_revision, updated_at
  )
  select
    (row->>'user_id')::bigint,
    (row->>'project_id')::bigint,
    row->>'module_key', row->>'submodule_key', row->>'exercise_key',
    row->>'question_key', coalesce(row->>'field_key', 'answer'),
    coalesce(row->'answer_value', 'null'::jsonb),
    nullif(row->>'source_version_id', '')::uuid,
    (row->>'client_updated_at')::timestamptz,
    coalesce((row->>'client_revision')::bigint, 0),
    now()
  ...
  do update set
    ...
    updated_at = now()
  where
    public.user_answers.client_updated_at < excluded.client_updated_at
    or (
      public.user_answers.client_updated_at = excluded.client_updated_at
      and public.user_answers.client_revision <= excluded.client_revision
    );
end;
$$;
