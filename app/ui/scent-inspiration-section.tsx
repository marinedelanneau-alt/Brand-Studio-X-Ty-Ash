  ...
  return (
    <article className="group min-w-0">
      <div className="relative mb-5 pr-7 sm:pr-9">
        <div className="relative aspect-[4/5] overflow-hidden bg-[#eee6d8]">
        ...
        {images[1] ? (
          <div className="absolute -bottom-3 right-0 h-[29%] w-[42%] rotate-2 overflow-hidden border-[5px] border-[#fbf6ed] bg-[#eee6d8] transition-transform duration-500 motion-reduce:transition-none group-hover:-translate-y-1 group-hover:rotate-0">
            <Image src={images[1].src} alt={images[1].alt} fill sizes="180px" className="object-cover" />
          </div>
        ) : null}
      </div>

      <h3 className="font-[family-name:var(--font-cormorant)] text-[1.55rem] font-semibold leading-tight text-[#24364f]">{title}</h3>
      <p className="mt-2 text-[0.7rem] font-bold uppercase leading-5 tracking-[0.09em] text-[#625a52]">{sensations.join(" · ")}</p>
      ...
      <p className="mt-4 max-w-sm text-sm leading-6 text-[#5f544a]">
        <strong className="block font-extrabold text-[#cf7430]">Dans la communication →</strong>
...
export default function ScentInspirationSection() {
  return (
    <section aria-labelledby="scent-inspiration-title" className="my-10 border-y border-[#eadfca] py-9 sm:my-12 sm:py-11">
      <div className="max-w-2xl">
        <p className="font-more-sugar text-lg text-[#cf7430]">Quelques pistes pour commencer…</p>
        <h2 id="scent-inspiration-title" className="mt-1 font-[family-name:var(--font-cormorant)] text-3xl font-semibold leading-tight text-[#24364f] sm:text-4xl">Pour t’aider à sentir l’idée…</h2>
        <p className="mt-4 text-sm leading-6 text-[#5f544a] sm:text-base sm:leading-7">Une odeur raconte souvent beaucoup plus qu’un parfum. Elle évoque une matière, une lumière, une ambiance, une émotion. Et tout cela peut ensuite se traduire dans ta communication.</p>
      </div>
      ...
      <div className="mt-10 max-w-3xl border-l-2 border-[#e6c9a2] pl-5">
        <p className="text-sm italic leading-6 text-[#6b625a]">Ce ne sont que des pistes. Ton univers peut sentir le café, la pluie, le cuir, le pain chaud, la mer, un jardin après l’orage… ou quelque chose de complètement différent.</p>
        <p className="font-more-sugar mt-3 text-lg text-[#cf7430]">Et la tienne, elle sentirait quoi ? ↘</p>
      </div>
    </section>
  );
}
