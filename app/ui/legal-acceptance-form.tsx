...
export default function LegalAcceptanceForm({ documentId }: { documentId: string }) {
  ...
  return <form action={acceptCurrentTerms} className="mt-7 space-y-5">
    ...
    <label className="flex items-start gap-3 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4">
    ...
    <button disabled={!accepted} className="w-full rounded-xl bs-button-primary px-5 py-4 font-bold text-[var(--tyash-text-on-primary)] disabled:cursor-not-allowed disabled:opacity-40">Accepter et continuer</button>
  </form>;
}
