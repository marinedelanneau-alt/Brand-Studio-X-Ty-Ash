  ...
  return (
    <article className="border-t border-[var(--border)] pt-5">
      <div className="flex items-start justify-between gap-4">
        <div><p className="text-[0.65rem] font-black tracking-[0.2em] text-[var(--tyash-label-text)]">{String(index + 1).padStart(2, "0")}</p><h3 className="mt-2 font-[family-name:var(--font-cormorant)] text-4xl font-semibold leading-none text-[var(--heading-color)]">{color.name}</h3></div>
        <p className="max-w-48 text-right font-[family-name:var(--font-cormorant)] text-lg italic leading-tight text-[var(--text-primary)]">{color.feeling}</p>
      </div>
      ...
      <div className="mt-5 grid grid-cols-4 gap-1.5" aria-label={`Nuances de ${color.name}`}>
        {color.shades.map((shade) => <div key={shade.name} tabIndex={0} title={shade.name} className="group relative h-11 outline-none ring-[var(--tyash-focus-ring)] focus-visible:ring-2" style={{ backgroundColor: shade.hex }}><span className="pointer-events-none absolute inset-x-0 bottom-full z-10 mx-auto mb-1 hidden w-max max-w-28 bg-[#24364f] px-2 py-1 text-center text-[0.58rem] font-bold uppercase tracking-wide text-white group-hover:block group-focus:block">{shade.name}</span></div>)}
      </div>
      <p className="mt-4 text-[0.64rem] font-black uppercase leading-5 tracking-[0.11em] text-[var(--text-primary)]">{color.words.join(" · ")}</p>
      <p className="font-more-sugar mt-3 text-base leading-5 text-[var(--tyash-label-text)]">{color.note}</p>
    </article>
  );
}
  ...
  return (
    <div className="overflow-hidden text-[var(--text-primary)]">
      <section className="pb-14 sm:pb-20">
        <h2 className="font-[family-name:var(--font-cormorant)] text-[clamp(2.4rem,6vw,4.8rem)] font-semibold leading-[0.92] tracking-[-0.04em] text-[var(--heading-color)]">Avant les mots,<br />il y a déjà une <em className="text-[var(--tyash-label-text)]">impression.</em></h2>
        ...
        <blockquote className="mt-9 border-l-2 border-[var(--tyash-primary)] py-2 pl-5 font-[family-name:var(--font-cormorant)] text-3xl font-semibold text-[var(--heading-color)] sm:text-4xl">Quelle impression veux-tu laisser ?</blockquote>
        <p className="font-more-sugar mt-4 text-lg text-[var(--tyash-label-text)]">On part de là ↓</p>
      </section>

      <section className="py-12 sm:py-16">
        <p className="text-[0.66rem] font-black uppercase tracking-[0.2em] text-[var(--tyash-label-text)]">La symbolique des couleurs</p>
        <h2 className="mt-3 font-[family-name:var(--font-cormorant)] text-3xl font-semibold leading-tight text-[var(--heading-color)] sm:text-4xl">Une couleur ne dit jamais<br />une seule chose.</h2>
        ...
        <p className="font-more-sugar mt-5 text-lg text-[var(--tyash-label-text)]">Même couleur. Pas la même histoire.</p>
      </section>
      ...
      <section aria-label="Bibliothèque des couleurs" className="py-14 sm:py-20">
        ...
        <div className="my-20 py-10 text-center">
          <h2 className="font-[family-name:var(--font-cormorant)] text-4xl font-semibold text-[var(--heading-color)]">La nuance change tout.</h2>
          <div className="mt-8 grid grid-cols-2 gap-2 sm:grid-cols-4">{[{ n: "Vert sauge", w: "calme", h: "#a9b7a0" }, { n: "Vert forêt", w: "naturel", h: "#244d35" }, { n: "Vert émeraude", w: "premium", h: "#13745d" }, { n: "Vert acidulé", w: "énergique", h: "#8fbd32" }].map((item) => <div key={item.n} className="flex aspect-square flex-col justify-end p-3 text-left text-white" style={{ backgroundColor: item.h, color: ["#a9b7a0", "#8fbd32"].includes(item.h) ? "var(--text-on-light-primary)" : "var(--text-on-dark-primary)" }}><p className="text-[0.62rem] font-black uppercase tracking-wide">{item.n}</p><p className="mt-1 font-[family-name:var(--font-cormorant)] text-xl">« {item.w} »</p></div>)}</div>
          <p className="font-more-sugar mt-5 text-lg text-[var(--tyash-label-text)]">Ne choisis donc jamais juste « du vert ».</p>
        </div>
      ...
      <section className="py-16 sm:py-20"><p className="text-[0.66rem] font-black uppercase tracking-[0.2em] text-[var(--tyash-label-text)]">L’antisèche</p><h2 className="mt-3 font-[family-name:var(--font-cormorant)] text-4xl font-semibold text-[var(--heading-color)]">À garder sous la main.</h2><p className="mt-4 max-w-2xl text-sm leading-7">Quelques repères pour comparer rapidement les grands territoires associés aux couleurs.</p>{recap}</section>

      <section className="py-16 text-center sm:py-24">
        <p className="text-[0.66rem] font-black uppercase tracking-[0.2em] text-[var(--tyash-label-text)]">Avant de choisir</p>
        <p className="mx-auto mt-6 max-w-3xl font-[family-name:var(--font-cormorant)] text-3xl leading-tight text-[var(--text-muted)] sm:text-5xl">Ne pars pas de :<br />« Quelle couleur j’aime ? »</p><p className="my-6 text-3xl text-[var(--tyash-label-text)]">↓</p>
        <p className="mx-auto max-w-3xl font-[family-name:var(--font-cormorant)] text-4xl font-semibold leading-tight text-[var(--heading-color)] sm:text-6xl">Pars de :<br />« Quelle <em className="text-[var(--tyash-label-text)]">impression</em> je veux laisser ? »</p>
        <div className="mt-12 grid grid-cols-1 gap-6 md:grid-cols-3">{["Ce que ta marque doit faire ressentir", "La personnalité que tu veux affirmer", "Les couleurs qui fonctionnent ensemble"].map((label, index) => <div key={label} className="border-t border-[var(--border)] pt-4 text-left"><p className="text-[0.65rem] font-black tracking-[0.2em] text-[var(--tyash-label-text)]">0{index + 1}</p><p className="mt-4 text-xs font-black uppercase leading-5 tracking-[0.1em] text-[var(--heading-color)]">{label}</p></div>)}</div>
      </section>

      <section className="py-14 sm:py-20"><h2 className="font-[family-name:var(--font-cormorant)] text-4xl font-semibold text-[var(--heading-color)]">Une palette, c’est une équipe.</h2><div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">{[{ n: "Fond", h: "#f5eddf", t: "#24364f" }, { n: "Principale", h: "#24364f", t: "white" }, { n: "Accent", h: "#cf7430", t: "white" }, { n: "Contraste", h: "#f0cf55", t: "#24364f" }].map((item) => <div key={item.n}><div className="aspect-[4/3]" style={{ backgroundColor: item.h }} /><p className="mt-3 text-center text-[0.62rem] font-black uppercase tracking-[0.15em]" style={{ color: "var(--text-primary)" }}>{item.n}</p></div>)}</div><p className="font-more-sugar mt-6 text-lg text-[var(--tyash-label-text)]">Toutes les couleurs n’ont pas besoin de crier en même temps.</p></section>

      <section className="py-16 text-center sm:py-24"><p className="text-[0.66rem] font-black uppercase tracking-[0.2em] text-[var(--tyash-label-text)]">À retenir</p><h2 className="mx-auto mt-7 max-w-3xl font-[family-name:var(--font-cormorant)] text-[clamp(3rem,8vw,6rem)] font-semibold leading-[0.88] tracking-[-0.045em] text-[var(--heading-color)]">Ne choisis pas<br />une couleur.<br /><br />Choisis une<br /><em className="text-[var(--tyash-label-text)]">impression.</em></h2><p className="mt-8 text-sm text-[var(--text-primary)] sm:text-base">Et construis ta palette autour d’elle.</p><p className="font-more-sugar mt-4 text-xl text-[var(--tyash-label-text)]">À toi de jouer ↓</p></section>
    </div>
  );
}
