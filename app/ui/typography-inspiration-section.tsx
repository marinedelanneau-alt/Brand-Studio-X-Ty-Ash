  ...
  return (
    <div className="text-[#5f544a]">
      <section aria-labelledby="typography-introduction-title" className="pb-10 sm:pb-12">
        ...
        <blockquote className="mt-7 max-w-2xl border-l-2 border-[#cf7430] pl-5 font-[family-name:var(--font-cormorant)] text-2xl font-medium leading-tight text-[#24364f] sm:text-[1.8rem]">
          Ta typographie donne une voix à tes mots avant même qu’on les lise.
        </blockquote>
      </section>

      <section aria-labelledby="type-specimens-title" className="border-y border-[#eadfca] py-10 sm:py-12">
        ...
        <div className="mt-9 grid grid-cols-1 gap-y-10 md:grid-cols-2 md:gap-x-7 xl:grid-cols-3">
          {typeSpecimens.map((specimen) => (
            <article key={specimen.number} className="min-w-0 border-t border-[#cfc3b2] pt-4">
              <p className="text-[0.68rem] font-extrabold tracking-[0.2em] text-[#cf7430]">{specimen.number}</p>
              <p className={`${specimen.fontClass} ${specimen.wordClass} mt-8 max-w-full break-words text-[clamp(3.1rem,7vw,5.4rem)] leading-[0.82] text-[#24364f]`}>Imagine.</p>
              <p className="mt-8 text-[0.67rem] font-extrabold uppercase leading-5 tracking-[0.1em] text-[#625a52]">{specimen.qualities}</p>
              <p className="mt-3 max-w-sm text-sm leading-6">{specimen.description}</p>
            </article>
          ))}
        </div>
        <p className="font-more-sugar mt-7 text-lg text-[#cf7430] sm:ml-3">Ça change déjà l’ambiance, non ? ↗</p>
      </section>
      ...
      <section aria-labelledby="font-pairing-title" className="py-10 sm:py-12">
        ...
        <div className="relative mt-8 overflow-hidden border-y border-[#d9cbb9] bg-[#fbf6ed] px-5 py-8 sm:px-8 sm:py-10">
          <div className="grid gap-9 lg:grid-cols-[minmax(0,1.15fr)_minmax(15rem,0.85fr)] lg:items-end">
            <div>
              <p className="text-[0.66rem] font-black uppercase tracking-[0.2em] text-[#cf7430]">Titre</p>
              <p className="mt-4 max-w-xl font-[family-name:var(--font-cormorant)] text-[clamp(2.7rem,6vw,4.8rem)] font-semibold leading-[0.88] tracking-[-0.04em] text-[#24364f]">Ta marque mérite<br />d’être remarquée.</p>
              <p className="font-more-sugar mt-5 text-base text-[#cf7430]">Typographie de caractère → titres, accroches, mots forts</p>
            </div>
            <div className="border-t border-[#d9cbb9] pt-6 lg:border-l lg:border-t-0 lg:pl-8 lg:pt-0">
              <p className="text-[0.66rem] font-black uppercase tracking-[0.2em] text-[#cf7430]">Texte</p>
              <p className="mt-4 max-w-md font-[family-name:var(--font-manrope)] text-base leading-7 text-[#4f463f] sm:text-lg">Quelques mots pour raconter ton histoire simplement et donner envie de découvrir la suite.</p>
              <p className="font-more-sugar mt-5 text-base text-[#cf7430]">Typographie de lecture → paragraphes, informations, contenus</p>
            </div>
          </div>
          <p className="mt-8 text-center text-xs font-black uppercase tracking-[0.18em] text-[#7a7087]">Typographie principale <span className="px-2 text-[#cf7430]">+</span> Typographie secondaire</p>
        </div>
      </section>

      <section aria-labelledby="good-reflex-title" className="border-y border-[#eadfca] py-10">
        <p className="font-more-sugar text-lg text-[#cf7430]">Le bon réflexe</p>
        ...
        <div className="mt-6 grid grid-cols-1 gap-y-7 md:grid-cols-3 md:gap-x-7">
          {goodReflexes.map(([number, title, description]) => (
            <article key={number} className="border-t border-[#cfc3b2] pt-4">
              ...
              <h3 className="mt-4 font-[family-name:var(--font-cormorant)] text-2xl font-semibold text-[#24364f]">{title}</h3>
      ...
      <section aria-labelledby="typography-takeaway-title" className="border-t border-[#eadfca] pt-10 sm:pt-12">
        <p className="text-[0.7rem] font-black uppercase tracking-[0.2em] text-[#cf7430]">À retenir</p>
        <h2 id="typography-takeaway-title" className="mt-3 max-w-3xl font-[family-name:var(--font-cormorant)] text-2xl font-medium leading-tight text-[#24364f] sm:text-3xl">Une bonne typographie n’est pas seulement une belle typographie.</h2>
        ...
        <p className="mt-7 max-w-3xl border-l-2 border-[#cf7430] pl-5 font-[family-name:var(--font-cormorant)] text-xl font-semibold leading-snug text-[#24364f] sm:text-2xl">Si ma marque parlait uniquement par la forme de ses mots, quelle impression voudrais-je qu’elle donne ?</p>
        <p className="font-more-sugar mt-4 text-lg text-[#cf7430]">Garde cette sensation en tête pour la suite ↘</p>
      </section>
    </div>
  );
}
