  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl border border-[var(--border)] bg-[var(--card)] p-6">
        ...
        <h1 className="mt-3 text-4xl text-[var(--heading-color)]">
        ...
        {query.message ? (
          <p
            className={`mt-4 rounded-xl p-3 ${
              query.kind === "error"
                ? "bs-status-light bg-[#fff0ed] text-[var(--status-error-text)]"
        ...
        {loadError ? (
          <div className="mt-4 rounded-xl border border-[#efc6bf] bs-status-light bg-[#fff4f1] p-4">
            <p className="font-bold text-[var(--status-error-text)]">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section className="rounded-3xl border border-[#d9e6d5] bg-[var(--card)] p-6">
          <span className="rounded-full bs-status-light bg-[#eef6eb] px-3 py-1 text-xs font-black uppercase tracking-[.14em] text-[var(--status-success-text)]">
            En ligne
          </span>
          <h2 className="mt-4 text-2xl text-[var(--heading-color)]">Version publiée</h2>
          {published ? (
        ...
        <section className="rounded-3xl border border-[var(--border)] bg-[var(--card)] p-6">
          <span className="rounded-full bg-[var(--tyash-soft)] px-3 py-1 text-xs font-black uppercase tracking-[.14em] text-[var(--tyash-label-text)]">
            Visible uniquement par les administrateurs
          </span>
          <h2 className="mt-4 text-2xl text-[var(--heading-color)]">Brouillon actuel</h2>
          {draft ? (
            <>
              ...
              {previewEnabled ? (
                ...
                <p className="mt-4 rounded-xl bg-[var(--background)] p-3 text-sm">
              ...
              {draft.status === "draft" ? (
                <form action={markReleaseReady} className="mt-5 space-y-3">
                  ...
                  <button className="rounded-xl bs-button-primary px-4 py-2 font-bold text-[var(--tyash-text-on-primary)]">
                    Marquer comme prête à publier
                  </button>
                </form>
              ) : null}
              {draft.status === "ready" && activeSchedule ? (
                <div className="mt-5 rounded-2xl border border-[var(--tyash-border)] bg-[var(--tyash-soft)] p-4">
                  <p className="font-bold text-[var(--tyash-label-text)]">
                  ...
                  {activeSchedule.status === "scheduled" ? (
                    <form action={cancelScheduledRelease} className="mt-3">
                      ...
                      />
                      <button className="rounded-xl border border-[var(--tyash-border)] px-4 py-2 text-sm font-bold text-[var(--tyash-label-text)]">
                ...
                <div className="mt-5 space-y-5 rounded-2xl border border-[#efc6bf] bs-status-light bg-[#fff4f1] p-4">
                  <p className="font-bold text-[var(--status-error-text)]">
                  ...
                  <div className="border-t border-[#efc6bf] pt-5">
                    ...
                    <form action={scheduleReleaseForAllUsers} className="mt-3 space-y-3">
                      ...
                      <button className="rounded-xl border border-[#9d4e40] px-4 py-2 font-bold text-[var(--status-error-text)]">
      ...
      <section className="rounded-3xl border border-[var(--border)] bg-[var(--card)] p-6">
        <h2 className="text-2xl text-[var(--heading-color)]">Comparaison publié / brouillon</h2>
        {differences.length ? (
          <ul className="mt-4 space-y-2">
            {differences.map((difference, index) => (
              <li key={`${difference.stableKey}:${difference.kind}:${index}`} className="rounded-xl bg-[var(--tyash-subtle)] p-3 text-sm">
                <strong>{difference.kind}</strong> · {difference.entityType} · {difference.label}
              </li>
            ))}
          </ul>
        ) : <p className="mt-3 text-[var(--text-muted)]">Aucune différence détectée.</p>}
      ...
      <section className="rounded-3xl border border-[#d9e6d5] bg-[var(--card)] p-6">
        <h2 className="text-2xl text-[var(--heading-color)]">
          ...
          {schedules.length ? schedules.map((schedule) => (
            <article key={schedule.id} className="rounded-xl border p-4">
              ...
              {schedule.error_message ? (
                <p className="mt-2 text-sm font-bold text-[var(--status-error-text)]">
                  {schedule.error_message}
                </p>
              ) : null}
            </article>
          )) : (
            <p className="rounded-xl bg-[var(--background)] p-3 text-sm">
      ...
      <section className="rounded-3xl border border-[#d8dce6] bg-[var(--card)] p-6">
        <h2 className="text-2xl text-[var(--heading-color)]">Version de code</h2>
      ...
      <section className="rounded-3xl border border-[var(--border)] bg-[var(--card)] p-6">
        <h2 className="text-2xl text-[var(--heading-color)]">Fonctionnalités expérimentales</h2>
        ...
        {draft?.status === "draft" ? (
          <form action={saveDraftFeatureFlag} className="mt-5 grid gap-3 rounded-2xl bg-[var(--tyash-subtle)] p-4 sm:grid-cols-2">
            <input type="hidden" name="releaseId" value={draft.id} />
      ...
      <section className="rounded-3xl border border-[var(--border)] bg-[var(--card)] p-6">
        <h2 className="text-2xl text-[var(--heading-color)]">Historique</h2>
        <div className="mt-4 space-y-3">
          {releases.map((release) => (
            <article key={release.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border p-4">
              <div>
                ...
                <p className="text-sm text-[var(--text-muted)]">{release.status} · {date(release.updated_at)}</p>
              </div>
              ...
              <Link href={`/admin/releases?technical=${release.id}`} className="text-sm font-bold text-[var(--tyash-label-text)]">
