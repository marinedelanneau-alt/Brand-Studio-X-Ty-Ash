...
export default async function LegalAdminPage() {
  if (!isLegalPreviewEnabled()) return <div className="rounded-2xl border bg-[var(--card)] p-6">Ce module est désactivé hors environnement Preview.</div>;
  ...
  return <div className="mx-auto max-w-6xl space-y-6">
    <form action={prepareLegalDrafts}><button className="rounded-xl border bg-[var(--card)] px-4 py-3">Préparer les brouillons CGU, CGV, confidentialité et mentions légales</button></form>
    <Link href="/admin/privacy" className="inline-block underline">Traiter les demandes relatives aux données personnelles</Link>
    <header><p className="text-xs font-black uppercase tracking-widest text-[var(--tyash-label-text)]">Administration</p><h1 className="mt-2 text-3xl font-semibold">Documents juridiques</h1></header>
    ...
    {documents.map((doc) => <section key={doc.id} className="rounded-2xl border bg-[var(--card)] p-5">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><span className="rounded-full bg-[var(--background)] px-3 py-1 text-xs font-bold">{labels[doc.status]}</span><h2 className="mt-3 text-xl font-semibold">{doc.title} · {doc.version}</h2></div><Link className="underline" target="_blank" href={`/${doc.slug}?preview=${doc.id}`}>Prévisualiser</Link></div>
