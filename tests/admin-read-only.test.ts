...
const guideDownload = readFileSync(new URL("../app/admin/users/[id]/guide/download/route.ts", import.meta.url), "utf8");
...
describe("read-only admin user insights", () => {
  it("contains no answer write operation in the Admin data paths", () => {
    for (const source of [dataLayer, exportRoute, guidePage, guideDownload]) {
